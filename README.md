...
## 7. Получение списка IP адресов оператора/операторов.

Для получения списка IP адресов оператора в формате JSON необходимо сделать запрос:

```
https://wapclick.mobi/api/ip/list.json?operator=mts
````

В настоящее время список доступных операторов:
...
При необходимости получить IP адреса нескольких операторов в формате JSON необходимо сделать запрос:

```
https://wapclick.mobi/api/ip/list.json?operator=mts&operator=mf
````
